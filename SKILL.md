...
完整的方法论工具定义在 `extensions/methodology/` 目录下，每个工具一个文件。以下为总览表——在对话中按场景 Read 对应文件获取详细指导。

### 工具索引

| 工具 | 文件 | 一句话 | 何时 Read |
|------|------|------|----------|
| **第一性原理** | `first-principles.md` | 退到约束重新推导 | 解释设计为什么存在 |
...
| **时间性推理** | `temporal.md` | 为什么现在才出现？ | 条件相关 bug / 回看 |
| **实践论** | `practice.md` | 最小验证实验 | 分析完后追问验证 |
| **辩证法反思** | `dialectics.md` | 正题→反题→合题 | 阶段性收束/回看 |
| **奥卡姆剃刀** | `occam.md` | 先验证最便宜的假设 | 多假设竞争时 |
| **结构主义** | `structuralism.md` | 意义来自关系网络 | 分析模块架构时 |
| **历史方法** | `history.md` | 看演化路径 | 理解复杂现有系统 |
| **否定之否定** | `negation.md` | 吸收矛盾双方的合理部分 | 二元选择困境 |

### 加载规则

1. 场景命中上表「何时 Read」列时，用 Read 工具读取对应文件获取详细指导
2. 每条回复至少使用 **2 个以上** 方法论工具，并显式标注工具名（如 `[第一性原理]`）
3. 不强制每个工具都 Read——场景不匹配时不触发。但触发了就必须显式标注
