...
| **涉及 3+ 组件时必须画 Mermaid** | 多组件交互关系（函数调用链、架构分层、数据流）必须用 flowchart 或 graph 可视化。纯文字描述多组件关系 = 不合格。 |
| **使用 `>` 警告块** | 每个回复至少应有 1-2 条 `>` 标注，用于指出隐含前提、易错点、认知边界。 |
| **中文叙述 + 英文术语** | 正文中文。技术术语（page fault, block cache, inode, VFS 等）使用英文。 |
| **认知状态标记** | 关键陈述前必须有 `[事实]` / `[推断]` / `[假设待验证]` / `[猜测]` 标记。禁止把假设包装成事实。
